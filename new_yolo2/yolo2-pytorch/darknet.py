...
class Darknet19(nn.Module):
    def __init__(self):
        ...
        self.bboxloss_val = 0.0
        self.iouloss_val = 0.0
        self.clsloss_val = 0.0
    ...
    def forward(self, im_data, gt_boxes=None, gt_classes=None, dontcare=None,
        ...
        if self.training:
            ...
            # print("Printing bbox_pred",bbox_pred_np.shape[0])
            ...
            # print(bbox_pred,box_mask,num_boxes)
            self.bbox_loss = nn.MSELoss(size_average=False)(bbox_pred * box_mask, _boxes * box_mask) / num_boxes  # noqa
            # print(self.bbox_loss.cpu().data.numpy())
            self.iou_loss = nn.MSELoss(size_average=False)(iou_pred * iou_mask, _ious * iou_mask) / num_boxes  # noqa
            # print(self.iou_loss.cpu().data.numpy())

            class_mask = class_mask.expand_as(prob_pred)
            self.cls_loss = nn.MSELoss(size_average=False)(prob_pred * class_mask, _classes * class_mask) / num_boxes  # noqa
            # print(self.cls_loss.cpu().data.numpy())

            self.bboxloss_val = self.bbox_loss.cpu().data.numpy()

            self.iouloss_val = self.iou_loss.cpu().data.numpy()

            self.clsloss_val = self.cls_loss.cpu().data.numpy()

        return bbox_pred, iou_pred, prob_pred, self.bboxloss_val, self.iouloss_val, self.clsloss_val
    ...
    def _build_target(self, bbox_pred_np, gt_boxes, gt_classes, dontcare,
        ...
        #     print(bbox_pred_np[b], gt_boxes[b],gt_classes[b], dontcare[b], iou_pred_np[b])
        #print(bbox_pred_np[0], gt_boxes[0],gt_classes[0], dontcare[0], iou_pred_np[0])
        #print partial(_process_batch, size_index=size_index)
        #print ((bbox_pred_np[b], gt_boxes[b],gt_classes[b], dontcare[b], iou_pred_np[b])for b in range(bsize))
        #print map(partial(_process_batch, size_index=size_index),((bbox_pred_np[b], gt_boxes[b],gt_classes[b], dontcare[b], iou_pred_np[b])for b in range(bsize)))       
