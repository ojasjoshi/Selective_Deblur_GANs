...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], use_parallel = True, learn_residual = False):
	...
	if len(gpu_ids) > 0:
		netG.cuda(gpu_ids[0]) ### change due to cuda unexpected keywork argument 
...
def define_D(input_nc, ndf, which_model_netD,
	...
	if which_model_netD == 'basic':
		...
		raise NotImplementedError('Discriminator model name [%s] is not recognized' %
								  which_model_netD)
	if use_gpu:
		netD.cuda(gpu_ids[0]) ## remove device_id
...
class ResnetGenerator(nn.Module):
	...
	def forward(self, input):
		...
		if self.learn_residual:
			######### folloowing code for not using crop ########## Tong Lin 
			#print ("[networks.py]",input.size(), output.size())
			#print ("[networks.py]",type(output))
			input_row=input.size(2)
			input_coln=input.size(3)
			output=output[0:1,0:3,0:input_row,0:input_coln] ######## Tong Lin added 
			#print ("[networks.py] resize output",output.size())
			####### ends ####################################
			output = input + output
